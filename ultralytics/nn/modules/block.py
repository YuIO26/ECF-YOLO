...
class CoordinateAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return identity * a_h * a_w  # 空间加权

class MKP_RFCA(nn.Module):
    """融合MKP多核感知与RFCAConv注意力的模块"""
    def __init__(self, in_channels, out_channels, kernels=[3, 5, 7], reduction=16):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 坐标注意力增强
        attended = self.ca(fused)
        
        # 点卷积降维
        return self.pw_conv(attended)
...
class GhostModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, ratio=2):
        ...
        if in_channels > 0:
            # 计算通道调整因子
            multiplier = math.ceil(init_channels / in_channels)
            self.init_channels = min(multiplier * in_channels, out_channels)
        else:
            # 防止除零错误
        ...
        # 打印调试信息
        print(f"GhostModule: in={in_channels}, out={out_channels}, ratio={ratio}, " 
              f"adjusted init={self.init_channels}, ghost={self.ghost_channels}")
        ...
        self.primary_conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True)
        ) if self.init_channels > 0 else nn.Identity()
        ...
        self.cheap_operation = nn.Sequential(
            ...
            nn.ReLU(inplace=True)
        ) if self.ghost_channels > 0 else nn.Identity()
    
    def forward(self, x):
        x1 = self.primary_conv(x)
        x2 = self.cheap_operation(x1)
        return torch.cat([x1, x2], dim=1) if self.ghost_channels > 0 else x1
...
class CoordinateAttention(nn.Module):
    def __init__(self, inp, reduction=32):
        ...
        # 计算MIP通道数
        mip = max(8, inp // reduction)
        print(f"CoordinateAttention: inp={inp}, mip={mip}")
...
class Ghost_MKP_RFCA(nn.Module):
    def __init__(self, in_channels, out_channels, kernels=[3, 5], reduction=32):
        super().__init__()
        print(f"Initializing Ghost_MKP_RFCA: in={in_channels}, out={out_channels}, kernels={kernels}")
        ...
        for k in kernels:
            conv = nn.Sequential(
                GhostModule(in_channels, in_channels, kernel_size=k),
                nn.BatchNorm2d(in_channels),
                nn.ReLU(inplace=True)
            )
            self.dw_convs.append(conv)
        ...
        # 点卷积层 - 确保分组参数有效
        groups = 1  # 默认组数
        if out_channels >= 2:  # 只有输出通道≥2时才使用分组卷积
            groups = min(2, out_channels)
        ...
        self.pw_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True) if out_channels > 0 else nn.Identity()
        )
